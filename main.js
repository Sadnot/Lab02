...
function showCart() { 
    ...
    if(localStorage.length != 0){
        ...
        '<tr>' +
            ...
            '<td >'+name+'</td>'+
            '<td class="table-footer">'+number+'</td>'+
            '<td class="table-footer">'+price+'</td>'+
            '<td class="table-footer">'+thanhtien+'</td>'+
            '<td style="text-align:center" >' +
                '<a data-code="'+key+'" href="#">' +
                    '<button onclick="removeCart(\''+key+'\')">'
                        '<i class="header-input-icon2 fa fa-trash icon-pink"></i>' +
                    '</button>' +
                '</a>' +
            '</td>'+
        '</tr>';
        document.getElementById('cart-table').insertRow(1).innerHTML = tbody;
        };
console.log(tbody)
    }
}
